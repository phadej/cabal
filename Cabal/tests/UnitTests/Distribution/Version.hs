...
versionTests =
    ...
    , testGroup "normalise examples" normaliseExamples 
...
prop_normalise_caret_equiv vr = prop_equivalentVersionRange
    vr
    (New.normaliseVersionRange2 vr)

-- UnionVersionRanges (IntersectVersionRanges (EarlierVersion (mkVersion [2])) (LaterVersion (mkVersion [1]))) (UnionVersionRanges (LaterVersion (mkVersion [2])) (EarlierVersion (mkVersion [1])))
-- TODO: reassoc
...
normaliseExamples =
    ...
    , ex "^>=1 || ^>=2 || ^>= 3"         "^>=1 || ^>=2 || ^>=3"
    , ex "^>=4.7 || ^>=4.8 || ^>= 4.9"   ">=4.7 && <4.9 || ^>=4.9"
    , ex "^>=1.1.1 && ^>=1.1.2"          "^>=1.1.2"

    -- "regressions"
    , ex "(^>=3 || <3) && > 0"      ">0 && <3 || ^>=3"
    , ex "(^>=3.1 || <3.1) && > 0"  ">0 && <3.1 || ^>=3.1"
    , ex ">0 && ^>=3"               "^>=3"

    -- (UnionVersionRanges (IntersectVersionRanges (MajorBoundVersion (mkVersion [1])) (EarlierVersion (mkVersion [3]))) (OrLaterVersion (mkVersion [2])))
    , ex "(^>=1 && <3) || >=2"      "^>=1 || >=2 && <3"
    ]
  where
    ex :: String -> String -> TestTree
    ex inp outp = testCase inp $ do
        inp'  <- either fail pure $ eitherParsec inp
    
        assertEqual "norm" outp (prettyShow $ New.normaliseVersionRange2 inp')

-- TODO: IntersectVersionRanges (UnionVersionRanges (MajorBoundVersion (mkVersion [3])) (EarlierVersion (mkVersion [3]))) (LaterVersion (mkVersion [0]))
...
-- TODO
-- UnionVersionRanges (EarlierVersion (mkVersion [1])) (MajorBoundVersion (mkVersion [0]))
-- mkVersion [1]
...
prop_gen_intervals1 i
  ...
  where
    ...
    xs = unVersionIntervals i
    ys = unVersionIntervals i -- 
